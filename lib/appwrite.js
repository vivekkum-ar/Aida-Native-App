...
/* ---------------------------------------------------------------------------------------------- */
/*                          Update name in the Auth and users collection                          */
/* ---------------------------------------------------------------------------------------------- */
export const editProfileData = async (updatedName, updatedUserName, updatedEmail, userPassword, docId) => {
  try {
    const resName = await account.updateName(updatedName);

    try {
      if(userPassword){
        const resEmail = await account.updateEmail(
          updatedEmail, // email
    ...
    const resUsername = await databases.updateDocument(
      config.databaseId,
      config.userCollectionId,
      docId,
      {
        username: updatedUserName,
        /* ------------------------- Required because we have to change in auth ------------------------- */
        /* ---------------------------- record as well as in users collection --------------------------- */
        email: updatedEmail,
      }
    );
