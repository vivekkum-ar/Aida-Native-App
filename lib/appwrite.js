...
export const getUserPost = async (userId) => {
  ...
  try {
    const posts = await databases.listDocuments(
      ...
      config.videoCollectionId,
      // [Query.sort("createdAt", "DESC")]
      [Query.equal("createdByUser", userId),Query.limit(7)]
    );
...
export const getLikedPost = async (userId,lastId,firstSet) => {
  ...
  try {
    ...
    const posts = await databases.listDocuments(
      ...
      config.videoCollectionId,
      // [Query.sort("createdAt", "DESC")]
      [Query.equal("likedByUsers", userId),Query.limit(20)]
    );
    console.log("id",posts.documents)
    return posts.documents;
  }else{
    const posts = await databases.listDocuments(
      ...
      config.videoCollectionId,
      // [Query.sort("createdAt", "DESC")]
      [Query.equal("likedByUsers", userId),Query.limit(20),Query.cursorAfter(lastId)]
    );
    console.log("id",posts.documents)
    return posts.documents;
  }
...
export const likePost = async (docId, previousLikesArray,userId) => {
  ...
  try {
    const updatedDoc = await databases.updateDocument(
      ...
      docId,
      {
        /* ------------ used ternary if to check if the user has already liked the post ----------- */
        likedByUsers: previousLikesArray.includes(userId) ? [...previousLikesArray] : [...previousLikesArray, userId.toString()],
      }
    )
